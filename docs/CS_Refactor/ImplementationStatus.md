...
- [x] **GenerateMipmap scalar 命令**：wire 增加 `GenerateMipmap{target}`；`Client::SendGenerateMipmap` + `ServerCore` 解包调 BFA `GenerateMipmap`；`ClientGenerateMipmapTest` 1/1 通过（0xDEAD 精确回查）
- [x] DirectGLES / DirectVulkan null adapter 提供 `Initialize/Shutdown/Display/SharedGroup/Session/Clear/ClearColor/DrawArrays/DrawElements/BufferSubData/MemoryBarrier/PatchParameteri/GenerateMipmap` 存根
