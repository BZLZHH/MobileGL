...
- [x] **ClientWaitSync + Query 命令族**：wire 增加 `ClientWaitSync{sync,flags,timeout}`（Response.sync 回传结果码）+ `BeginTimeElapsedQuery/EndTimeElapsedQuery/DeleteBackendQuery`；`Client::SendClientWaitSync/SendBeginTimeElapsedQuery/SendEndTimeElapsedQuery/SendDeleteBackendQuery`；`ServerCore` 分发（glClientWaitSync/glBeginQueryIndexed/glEndQueryIndexed/glDeleteQueries → BFA ClientWaitSync/BeginTimeElapsedQuery/EndTimeElapsedQuery/DeleteBackendQuery）；`ClientClientWaitSyncTest` 1/1（result=0x911B、query handle=0x7777、end/delete 回查）
