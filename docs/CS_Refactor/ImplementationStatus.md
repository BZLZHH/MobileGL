...
- [x] **DrawArraysInstanced 命令**：wire 增加 `DrawArraysInstanced{mode,first,count,primcount}`；`Client::SendDrawArraysInstanced` + `ServerCore` 解包调 BFA `DrawArraysInstanced`（int32 instanceCount）；`ClientDrawArraysInstancedTest` 1/1 通过（4/3/6/2 精确回查）
- [x] DirectGLES / DirectVulkan null adapter 提供 `` 存根
