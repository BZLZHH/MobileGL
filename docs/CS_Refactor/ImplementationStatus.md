...
- **全量 ctest（unit）**：1422/1422 通过（3 skipped）✅（含 ClientDispatchComputeIndirect 等新测试）
...
- [x] **WaitSync 命令**：wire 增加 `WaitSync{sync,flags,timeout}`；`Client::SendWaitSync` + `ServerCore` 转 `void*` 调 BFA `WaitSync`；`ClientWaitSyncTest` 1/1 通过（sync=0x1234/flags=0x33/timeout 精确回查）
- [x] DirectGLES / DirectVulkan null adapter 提供 `` 存根
