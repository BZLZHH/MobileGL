...
- [x] **DispatchComputeIndirect 命令**：wire 增加 `DispatchComputeIndirect{indirect_offset}`；`Client::SendDispatchComputeIndirect` + `ServerCore` 解包调 BFA；`ClientDispatchComputeIndirectTest` 1/1 通过（0x12345678 精确回查）
- [x] DirectGLES / DirectVulkan null adapter 提供 `` 存根
