...
- **全量 ctest（unit）**：1427/1427 通过（3 skipped）✅（含 ClientDrawElementsInstanced 等新测试）
...
- [x] **DrawElementsInstanced（shm indices + instanceCount）**：wire 增加 `DrawElementsInstanced{mode,count,type,indices_offset,instance_count}`；`Client::SendDrawElementsInstanced` 携带 shm fd；`ServerCore` 映射后把 indices 指针 + instanceCount 传给 BFA；`ClientDrawElementsInstancedTest` 1/1 通过（offset=1 读到 0x2B + 7 次实例）
- [x] DirectGLES / DirectVulkan null adapter 提供 `` 存根
