...
> 最新 HEAD：`cc4b7b9f [Feat] (MG_Protocol, MG_Client, MG_FullServer, DirectGLES): Add TextureSubImage shm upload command.`（TextureSubImage 提交后）
...
- **全量 ctest（unit）**：1435/1435 通过（3 skipped）✅（含 BufferRespecify / Indirect / GetString / TextureRespecify / TextureSubImage / ReadPixels 服务端→客户端 shm 新测试）
