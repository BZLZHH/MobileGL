...
- [x] **FullServer 侧 BFA shim 初版**：`MobileGL/MG_FullServer/BfaFrontendShim.h/.cpp` —— 把旧 `gBackendFunctionsTable.GL` 的 Clear/DrawArrays/DrawElements/DrawArraysInstanced/DrawElementsInstanced/DrawRangeElements/BlitFramebuffer/GenerateMipmap/DispatchCompute/DispatchComputeIndirect/MemoryBarrier/MemoryBarrierByRegion/FenceSync/DeleteSync/WaitSync/Present 转发到 BFA vtable；`ServerCore` 增加 `SetSessionListener`（session create/destroy 通知），`FullServerEntry` 在 start 时 `Install`、session 变化时更新 current session、destroy 时 `Clear`；`BfaFrontendShimTest` 1/1 通过，BigServerE2E/ClientSessionLifecycle 1/1，真实 DirectGLES Python E2E status=0
