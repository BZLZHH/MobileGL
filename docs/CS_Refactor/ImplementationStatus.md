...
- [x] **DispatchCompute scalar 命令**：wire 增加 `DispatchCompute{num_groups_x/y/z}`；`Client::SendDispatchCompute` + `ServerCore` 解包调 BFA `DispatchCompute`；`ClientDispatchComputeTest` 1/1 通过（3/4/5 精确回查）
- [x] DirectGLES / DirectVulkan null adapter 提供 `Initialize/Shutdown/Display/SharedGroup/Session/Clear/ClearColor/DrawArrays/DrawElements/BufferSubData/MemoryBarrier/PatchParameteri/GenerateMipmap/DispatchCompute` 存根
