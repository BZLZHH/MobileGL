...
- [x] **SwapBuffers 命令**：wire 增加 `SwapBuffers{draw}`；`Client::SendSwapBuffers` + `ServerCore` 调 BFA `SwapBuffers`（bool 决定 status）；`ClientSwapBuffersTest` 1/1 通过（0xCAFE 精确回查）
- [x] DirectGLES / DirectVulkan null adapter 提供 `` 存根
