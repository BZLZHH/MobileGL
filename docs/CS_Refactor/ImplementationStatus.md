...
- [x] **BeginTransformFeedback scalar 命令**：wire 增加 `BeginTransformFeedback{primitive_mode}`；`Client::SendBeginTransformFeedback` + `ServerCore` 解包调 BFA `BeginTransformFeedback`；`ClientBeginTransformFeedbackTest` 1/1 通过（0x55 精确回查）
- [x] DirectGLES / DirectVulkan null adapter 提供 `Initialize/Shutdown/Display/SharedGroup/Session/Clear/ClearColor/DrawArrays/DrawElements/BufferSubData/MemoryBarrier/PatchParameteri/GenerateMipmap/DispatchCompute/BeginTransformFeedback` 存根
