...
// first (and only) place that completes it for the DirectVulkan adapter.
struct MobileGLBackend {
    const MobileGLBackendVTable* VTable;
};

// Phase 5 wires the real DirectVulkan backend behind the BFA vtable. The null
// adapter below lets FullServer negotiate ABI, create a backend and run the
// lifecycle without a GPU; the real DirectVulkan sources replace it in Phase 5.
namespace {
    ...
    void DrawRangeElementsBackend(MobileGLBackend* self, MobileGLSessionId session,
