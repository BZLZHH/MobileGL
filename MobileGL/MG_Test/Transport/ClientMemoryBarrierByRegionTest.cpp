// MobileGL - MobileGL/MG_Test/Transport/ClientMemoryBarrierByRegionTest.cpp
...
namespace {
    ...
    void TestMemoryBarrierByRegion(MobileGLBackend* backend, MobileGLSessionId session,
                                   uint32_t barriers) {
        (void)backend;
        (void)session;
        g_barriers = barriers;
...
namespace MobileGL::Transport {
    TEST(ClientMemoryBarrierByRegionTest, ScalarPayload) {
        g_barriers = 0;
        ...
        vtable.MemoryBarrierByRegion = &TestMemoryBarrierByRegion;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(20, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        EXPECT_TRUE(Client::SendMemoryBarrierByRegion(20, 0x5678, 2));

        serverThread.join();
        EXPECT_TRUE(serverOk);
        EXPECT_EQ(g_barriers, 0x5678u);
