// MobileGL - MobileGL/MG_Test/Transport/ClientTextureRespecifyTest.cpp
...
namespace {
    ...
    void TestTextureRespecify(MobileGLBackend* backend, MobileGLSessionId session, MobileGLBackendHandle texture,
        ...
        g_level = levels[0].level;
        g_format = levels[0].format;
        g_type = levels[0].type;
        g_width = levels[0].width;
        g_height = levels[0].height;
        g_depth = levels[0].depth;
        g_dataSize = levels[0].dataSize;
        g_firstDataByte = levels[0].data == nullptr ? 0 : *static_cast<const uint8_t*>(levels[0].data);
    }
} // namespace

namespace MobileGL::Transport {
    TEST(ClientTextureRespecifyTest, ShmPixelsReachBackend) {
        g_texture = 0;
        g_level = 0;
        g_format = 0;
        g_type = 0;
        ...
        const char* endpoint = "/tmp/mobilegl_texrespec_test.sock";
        unlink(endpoint);
        ...
        vtable.TextureRespecify = &TestTextureRespecify;
        ...
        ASSERT_TRUE(Client::SendTextureRespecify(10, 789, 2, 0x8058, 0x1401, 4, 3, 1, 2, &shm, 2));
        serverThread.join();

        EXPECT_TRUE(serverOk);
        EXPECT_EQ(g_texture, 789u);
        EXPECT_EQ(g_level, 2u);
        EXPECT_EQ(g_format, 0x8058u);
        EXPECT_EQ(g_type, 0x1401u);
        EXPECT_EQ(g_width, 4u);
        EXPECT_EQ(g_height, 3u);
        EXPECT_EQ(g_depth, 1u);
        EXPECT_EQ(g_dataSize, 2u);
        EXPECT_EQ(g_firstDataByte, 0x77u);
