// MobileGL - MobileGL/MG_Test/Transport/ClientDispatchComputeIndirectTest.cpp
...
namespace {
    ...
    void TestDispatchComputeIndirect(MobileGLBackend* backend, MobileGLSessionId session,
                                     int64_t indirectOffset) {
        (void)backend;
        (void)session;
        g_offset = indirectOffset;
    }
} // namespace

namespace MobileGL::Transport {
    TEST(ClientDispatchComputeIndirectTest, ScalarPayload) {
        g_offset = 0;
        ...
        vtable.DispatchComputeIndirect = &TestDispatchComputeIndirect;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(22, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        EXPECT_TRUE(Client::SendDispatchComputeIndirect(22, 0x12345678, 2));

        serverThread.join();
        EXPECT_TRUE(serverOk);
        EXPECT_EQ(g_offset, static_cast<int64_t>(0x12345678));
