// MobileGL - MobileGL/MG_Test/Transport/ClientDrawElementsShmTest.cpp
...
namespace {
    ...
    void TestDrawElements(MobileGLBackend* backend, MobileGLSessionId session,
...
namespace MobileGL::Transport {
    TEST(ClientDrawElementsShmTest, ShmIndicesReachBackend) {
        g_mode = 0;
        g_count = 0;
        g_type = 0;
        g_firstIndexByte = 0;

        const char* endpoint = "/tmp/mobilegl_drawelements_test.sock";
        unlink(endpoint);
        ...
        vtable.DrawElements = &TestDrawElements;
        ...
        ASSERT_TRUE(Client::SubmitSessionControl(9, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        ASSERT_TRUE(Client::SendDrawElements(9, 4, 3, 5, 1, &shm, 2));
        serverThread.join();
