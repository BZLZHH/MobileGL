// MobileGL - MobileGL/MG_Test/Transport/ClientBindTransformFeedbackTest.cpp
...
namespace {
    ...
    void TestBindTransformFeedback(MobileGLBackend* backend, MobileGLSessionId session,
                                   uint32_t name) {
        (void)backend;
        (void)session;
        g_name = name;
...
namespace MobileGL::Transport {
    TEST(ClientBindTransformFeedbackTest, ScalarPayload) {
        g_name = 0;
        ...
        vtable.BindTransformFeedback = &TestBindTransformFeedback;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(18, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        EXPECT_TRUE(Client::SendBindTransformFeedback(18, 0xABC, 2));

        serverThread.join();
        EXPECT_TRUE(serverOk);
        EXPECT_EQ(g_name, 0xABCu);
