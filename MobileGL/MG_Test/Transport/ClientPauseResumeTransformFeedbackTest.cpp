// MobileGL - MobileGL/MG_Test/Transport/ClientPauseResumeTransformFeedbackTest.cpp
...
namespace {
    uint32_t g_pauseCount = 0;
    uint32_t g_resumeCount = 0;
    ...
    void TestPause(MobileGLBackend* backend, MobileGLSessionId session) {
        (void)backend;
        (void)session;
        ++g_pauseCount;
    }

    void TestResume(MobileGLBackend* backend, MobileGLSessionId session) {
        (void)backend;
        (void)session;
        ++g_resumeCount;
    }
...
namespace MobileGL::Transport {
    TEST(ClientPauseResumeTransformFeedbackTest, OpcodeOnlyCommands) {
        g_pauseCount = 0;
        g_resumeCount = 0;
        ...
        vtable.PauseTransformFeedback = &TestPause;
        vtable.ResumeTransformFeedback = &TestResume;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(17, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        EXPECT_TRUE(Client::SendPauseTransformFeedback(17, 2));
        EXPECT_TRUE(Client::SendResumeTransformFeedback(17, 3));

        serverThread.join();
        EXPECT_TRUE(serverOk);
        EXPECT_EQ(g_pauseCount, 1u);
        EXPECT_EQ(g_resumeCount, 1u);
