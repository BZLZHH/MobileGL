// MobileGL - MobileGL/MG_Test/Transport/ClientPatchParameteriTest.cpp
...
namespace {
    ...
    void TestPatchParameteri(MobileGLBackend* backend, MobileGLSessionId session,
...
namespace MobileGL::Transport {
    TEST(ClientPatchParameteriTest, ScalarPayload) {
        g_pname = 0;
        g_value = 0;
        ...
        vtable.PatchParameteri = &TestPatchParameteri;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(12, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        EXPECT_TRUE(Client::SendPatchParameteri(12, 0x1234, -7, 2));

        serverThread.join();
        EXPECT_TRUE(serverOk);
        EXPECT_EQ(g_pname, 0x1234u);
        EXPECT_EQ(g_value, -7);
