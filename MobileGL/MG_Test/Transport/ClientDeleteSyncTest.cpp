// MobileGL - MobileGL/MG_Test/Transport/ClientDeleteSyncTest.cpp
...
namespace {
    ...
    void TestDeleteSync(MobileGLBackend* backend, MobileGLSessionId session, void* sync) {
        (void)backend;
        (void)session;
        g_deletedSync = sync;
    }
...
namespace MobileGL::Transport {
    TEST(ClientDeleteSyncTest, FenceThenDelete) {
        ...
        vtable.DeleteSync = &TestDeleteSync;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(24, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        uint64_t sync = 0;
        EXPECT_TRUE(Client::SendFenceSync(24, 0x11, 0x22, 2, &sync));
        EXPECT_EQ(sync, 0x1234u);
        EXPECT_TRUE(Client::SendDeleteSync(24, sync, 3));

        serverThread.join();
        EXPECT_TRUE(serverOk);
        EXPECT_EQ(reinterpret_cast<uint64_t>(g_deletedSync), 0x1234u);
