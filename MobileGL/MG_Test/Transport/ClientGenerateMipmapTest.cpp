// MobileGL - MobileGL/MG_Test/Transport/ClientGenerateMipmapTest.cpp
...
namespace {
    ...
    void TestGenerateMipmap(MobileGLBackend* backend, MobileGLSessionId session,
                            uint32_t target) {
        (void)backend;
        (void)session;
        g_target = target;
...
namespace MobileGL::Transport {
    TEST(ClientGenerateMipmapTest, ScalarPayload) {
        g_target = 0;
        ...
        vtable.GenerateMipmap = &TestGenerateMipmap;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(13, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        EXPECT_TRUE(Client::SendGenerateMipmap(13, 0xDEAD, 2));

        serverThread.join();
        EXPECT_TRUE(serverOk);
        EXPECT_EQ(g_target, 0xDEADu);
