// MobileGL - MobileGL/MG_Test/Transport/ClientDrawElementsInstancedTest.cpp
...
namespace {
    uint32_t g_mode = 0, g_type = 0;
    int32_t g_count = 0, g_instanceCount = 0;
    ...
    void TestDrawElementsInstanced(MobileGLBackend* backend, MobileGLSessionId session,
        ...
        g_count = count;
        g_type = type;
        g_instanceCount = instanceCount;
        g_firstIndexByte = indices == nullptr ? 0 : *static_cast<const uint8_t*>(indices);
    }
} // namespace

namespace MobileGL::Transport {
    TEST(ClientDrawElementsInstancedTest, ShmIndicesAndInstanceCount) {
        g_mode = 0; g_count = 0; g_type = 0; g_instanceCount = 0; g_firstIndexByte = 0;
        ...
        vtable.DrawElementsInstanced = &TestDrawElementsInstanced;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(28, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        EXPECT_TRUE(Client::SendDrawElementsInstanced(28, 4, 3, 5, 1, 7, &shm, 2));
        serverThread.join();
