// MobileGL - MobileGL/MG_Test/Transport/ClientFenceSyncTest.cpp
...
namespace MobileGL::Transport {
    TEST(ClientFenceSyncTest, HandleReturnPath) {
        g_condition = 0;
        g_flags = 0;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(23, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        uint64_t sync = 0;
        EXPECT_TRUE(Client::SendFenceSync(23, 0x11, 0x22, 2, &sync));

        serverThread.join();
        EXPECT_TRUE(serverOk);
        EXPECT_EQ(g_condition, 0x11u);
        EXPECT_EQ(g_flags, 0x22u);
