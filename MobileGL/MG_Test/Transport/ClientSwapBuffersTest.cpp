// MobileGL - MobileGL/MG_Test/Transport/ClientSwapBuffersTest.cpp
...
namespace {
    ...
    bool TestSwapBuffers(MobileGLBackend* backend, MobileGLSessionId session,
...
namespace MobileGL::Transport {
    TEST(ClientSwapBuffersTest, DrawHandlePayload) {
        g_draw = 0;
        ...
        vtable.SwapBuffers = &TestSwapBuffers;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(21, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        EXPECT_TRUE(Client::SendSwapBuffers(21, 0xCAFE, 2));

        serverThread.join();
        EXPECT_TRUE(serverOk);
        EXPECT_EQ(g_draw, 0xCAFEu);
