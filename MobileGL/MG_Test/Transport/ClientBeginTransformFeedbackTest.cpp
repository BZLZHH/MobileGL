// MobileGL - MobileGL/MG_Test/Transport/ClientBeginTransformFeedbackTest.cpp
...
namespace {
    ...
    void TestBeginTransformFeedback(MobileGLBackend* backend, MobileGLSessionId session,
                                    uint32_t primitiveMode) {
        (void)backend;
        (void)session;
        g_primitiveMode = primitiveMode;
    }
} // namespace

namespace MobileGL::Transport {
    TEST(ClientBeginTransformFeedbackTest, ScalarPayload) {
        g_primitiveMode = 0;
        ...
        vtable.BeginTransformFeedback = &TestBeginTransformFeedback;
        ...
        EXPECT_TRUE(Client::SubmitSessionControl(15, true, 1));
        EXPECT_TRUE(Client::WaitResponseForToken(1, 5000));
        EXPECT_TRUE(Client::SendBeginTransformFeedback(15, 0x55, 2));

        serverThread.join();
        EXPECT_TRUE(serverOk);
        EXPECT_EQ(g_primitiveMode, 0x55u);
